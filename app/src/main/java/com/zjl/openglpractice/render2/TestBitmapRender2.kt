...
class TestBitmapRender2(
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        curProgram = createProgram(getMyVertexShader(), getFragmentShader())
    ...
    override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
        ...
        //Matrix.setRotateM(mMVPMatrix, 0, 180f, 0f, 0f, 1.0f)
        Matrix.scaleM(
            mMVPMatrix,
            0,
        ...
        Matrix.translateM(
            mMVPMatrix,
            0,
            (mGLSurfaceView!!.getWidth() / (mWidth) - 1.0f),
            -(mGLSurfaceView!!.getHeight() / (mHeight) - 1.0f),
            0f
        )
    }
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        GLUtils.texImage2D(
            GLES20.GL_TEXTURE_2D,
            0,
            BitmapFactory.decodeResource(context.resources, R.drawable.mute),
            0
        )
        //绑定注入bitmap
        val mFilterInputTextureUniform2 = GLES20.glGetUniformLocation(curProgram, "sTexture2")
    ...
    private fun getFragmentShader(): String {
        return "#extension GL_OES_EGL_image_external : require\n" +
                ...
                "  vec4 c2 = texture2D(sTexture2, vTextureCoord);\n" +
                "  if (c1.a != 0.0) {\n" +
                "      gl_FragColor = vec4(c1.rgb, c1.a *" + mAlpha + ");\n" +
                "  }else{\n" +
                "       gl_FragColor = vec4(c2.rgb, c2.a *" + mAlpha + ");\n" +
                "  }\n" +
                "}\n";
    }


    private fun getMyVertexShader(): String {
